...
class ChatApiController extends Controller
{
    ...
    public function store(Request $request){
        ...
        if($isUploadImage){
            ...
            while (!$body->eof()) {
                ...
                while (($pos = strpos($buffer, "\n")) !== false) {
                    ...
                    $fullText .= $chunk;
                    $fullThink .= $think;
                    broadcast(new AiChatEvent($chunk, $think, $done));
                }
            }
        }else{
            $response = Http::withOptions([
        ...
        // dd($messages);
        $finalResponse = Http::withOptions([
            'stream' => true
        ])->timeout(300)->post("http://localhost:11434/api/chat", [
            'model' => 'qwen3.5:4b',
            'stream' => true,
        ...
        $finalBody = $finalResponse->toPsrResponse()->getBody();
        $finalBuffer = '';
        $finalFullText = "";
        $finalFullThink = "";
        while (!$finalBody->eof()) {
            $finalBuffer .= $finalBody->read(1024);
            while (($pos = strpos($finalBuffer, "\n")) !== false) {
                $line = substr($finalBuffer, 0, $pos);
                $finalBuffer = substr($finalBuffer, $pos + 1);
                $line = trim($line);
                if (empty($line)) {
                    continue;
                }
